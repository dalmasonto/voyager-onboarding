		...
		try {
			...
			db.run(`INSERT INTO blocks (${columns}) VALUES(${values})`, undefined, (err) => {
				if (err) {
					...
					console.log(`Successfully inserted block: ${current_block}`);
				}
			})
		} catch (error) {
			console.error(`Error syncing block ${current_block}:`, error);
		}
	}
}
...
const chunking = async (start: number, end: number) => {
	...
	if (end - start > CHUNK_SIZE) {
		for (; task_index > start + CHUNK_SIZE; task_index -= CHUNK_SIZE) {
			tasks.push(syncBlockRange(task_index - CHUNK_SIZE + 1, task_index))
		}
		if (task_index > start) {
			tasks.push(syncBlockRange(start, task_index))
		}
	} else {
...
const runBlocks = async () => {
	try {
		...
		db.get(query, undefined, (err: any, row: any) => {
			if (err) {
				console.error("Failed to get the latest synced block", err);
			} else {
